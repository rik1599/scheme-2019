...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |laboratorio 22_11_2018 (2)|) (read-case-sensitive #t) (teachpacks ((lib "drawing.ss" "installed-teachpacks"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "drawing.ss" "installed-teachpacks")) #f)))
...
(define L
  (lambda (tile passi)
    (glue-tiles 
     (glue-tiles tile (shift-down (quarter-turn-left tile) passi))
     (glue-tiles (shift-right (shift-down tile (/ passi 2)) (/ passi 2)) (shift-right (quarter-turn-right tile) passi))
)))
...
(define L-tesselation
  (lambda (n)
    (cond ((= n 1) L-tile)
          ((= n 2) (L L-tile n))
          (else (L (L-tesselation (quotient n 2)) n))
    )))
